...
const EndedLotteries = () => {
  ...
  return (
    <div className="ended-lotteries">
      <h1>Ended Lotteries</h1>
      {loading && <p>Loading...</p>}
      {error && <p>Error: {JSON.stringify(error)}</p>}
      {!loading && (lotteries.length === 0 || lotteries[0] === "") && <p>No ended lotteries found.</p>}
      {(lotteries.length > 0 && lotteries[0] !== "") && (
        <ul>
          {lotteries.map((lottery, index) => (
            <li key={index}>{lottery}</li>
          ))}
        </ul>
      )}
    </div>
